<?php
  ...
  if (isset($_POST["submit"])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($row['role'] !== 'Utility') {
          $_SESSION['login_error'] = 'Access restricted to Utility users only.';
...
<html lang="en">
  ...
  <body>
    <div class="w-full max-w-4xl mx-4 flex flex-col md:flex-row rounded-xl overflow-hidden shadow-lg mt-10">
      <!-- Left - Branding -->
      <div class="w-full md:w-2/5 bg-blue-800 flex items-center justify-center p-6">
        <div class="text-center">
          <div class="logo-pulse mb-4">
            <img src="nobglogo.png" width="500" height="200" class="text-white mx-auto" />
          </div>

          <h1 class="text-3xl font-bold text-white mb-2">Utility</h1>
          ...
          <div class="mt-6 p-4 bg-white bg-opacity-20 rounded-lg backdrop-blur-sm">
            <p class="text-blue-50 text-sm">
              "Modernizing the Art of <i>'Oops, I Forgot That!'</i>—bringing quick, clever solutions to life's little slip-ups."
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="login-container w-full md:w-3/5 p-6">
        <form id="loginForm" class="login-form space-y-4" method="POST" action="">
          ...
          <div class="input-box">
            ...
            <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($email_value); ?>" class="form-input w-full px-3 py-2 border <?php echo !empty($field_errors['email']) ? 'border-red-500' : 'border-blue-300'; ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-blue-800 text-sm" placeholder="you@example.com" required/>
            
            <?php if (!empty($field_errors['email'])): ?>
              <div class="text-red-600 text-sm mt-1"><?php echo $login_error; ?></div>
            <?php endif; ?>
          </div>
        ...
        <!-- Password Input -->
          <div class="input-box password-container">
            ...
            <input type="password" name="password" id="password" class="form-input w-full px-3 py-2 border <?php echo !empty($field_errors['password']) ? 'border-red-500' : 'border-blue-300'; ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-blue-800 text-sm" placeholder="Enter your password" required/>
        ...
        <div id="successMessage" class="hidden mt-4 p-3 rounded-lg bg-green-100 text-green-700 text-center text-sm"> Login successful! Redirecting... </div>
      </div>
    </div>
    <script>
