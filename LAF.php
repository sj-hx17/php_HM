<?php 
    ...
    $lostCount = 0;
    $lostQuery = "SELECT COUNT(*) as count FROM laf WHERE status = 'Lost'";
    $lostResult = $connect->query($lostQuery);
    if ($lostResult) {
        $lostRow = $lostResult->fetch_assoc();
        $lostCount = $lostRow['count'];
    }

    // Count the number of found items
    $foundCount = 0;
    $foundQuery = "SELECT COUNT(*) as count FROM laf WHERE status = 'Found'";
    $foundResult = $connect->query($foundQuery);
    if ($foundResult) {
        $foundRow = $foundResult->fetch_assoc();
        $foundCount = $foundRow['count'];
    }
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <section id="content">
        ...
        <main>
            ...
            <div class="table-data">
                <div class="order">
                    <div class="head">
                        <h3>Lost and Found Directory</h3>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/4">Item</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">Date</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">Location</th>
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">Status</th>
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">Action</th>
                                </tr>
                            </thead>

                            <tbody id="lost-items-table" class="bg-white divide-y divide-gray-200">
                                <?php
                            $sql = "SELECT * FROM laf";

                            if (!empty($search)) {
                                $sql .= " WHERE item LIKE '%$search%' OR date LIKE '%$search%' OR location LIKE '%$search%'";
                            }

                            $result = $connect->query($sql);
                            $count = $connect->query("SELECT COUNT(*) FROM laf" . (!empty($search) ? " WHERE item LIKE '%$search%' OR date LIKE '%$search%' OR location LIKE '%$search%'" : ""))->fetch_row()[0];
                                ...
                                while($row = $result->fetch_assoc()){
                                    ...
                                    <tr class='hover:bg-gray-50 transition-colors duration-150'>
                                        ...
                                        <td class='px-6 py-4'>
                                            <div class='flex items-center'>
                                                <div class='ml-4'>
                                                    <div class='text-sm font-medium text-gray-900'>{$row['status']}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class='px-6 py-4 text-right'>
                                            <div class='flex justify-end space-x-2'>
                                                <a href='#' " . ($row['status'] == 'Found' ? "class='text-gray-400 cursor-not-allowed mr-3'" : "class='text-blue-600 hover:text-blue-900 mr-3' onclick='markItemAsFound({$row['id']})'") . ">
        ...
        <script>
            ...
            if (confirm("Are you sure you want to mark this item as 'Found'?")) {
                fetch('updateItemStatus.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    body: 'id=' + itemId + '&status=Found'
                })
                .then(response => {
                    if (response.ok) {
